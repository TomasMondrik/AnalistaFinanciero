  ...
  return (
    <main className="min-h-screen pb-20 antialiased bg-[#070A11] text-slate-100">
        ...
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center px-4">
          <div className="bg-[#0F1626] border border-sky-500/30 rounded-2xl w-full max-w-lg p-6 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div>
                <span className="text-[10px] font-mono uppercase bg-sky-500/10 text-sky-400 px-2 py-0.5 rounded border border-sky-500/20">
                  {selectedThesis.status}
                </span>
                <h3 className="text-lg font-bold text-slate-100 mt-1">
                  {selectedThesis.ticker} - {selectedThesis.name}
                </h3>
              </div>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <span className="text-slate-400 block font-medium">Potencial Estimado por Gemini (Upside):</span>
                <p className="text-emerald-400 font-mono font-bold text-sm">{selectedThesis.target}</p>
              </div>
              <div className="bg-slate-900/90 p-3 rounded-lg border border-slate-800">
                <span className="text-amber-400 font-semibold block mb-1">Argumentación de la IA:</span>
                <p className="text-slate-300 leading-relaxed">{selectedThesis.thesis}</p>
              </div>
            </div>
            <button
              onClick={() => setSelectedThesis(null)}
              className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 py-2 rounded-xl text-xs font-semibold"
            >
              Cerrar Tesis
            </button>
          </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="font-bold text-base text-slate-100 tracking-tight flex items-center gap-2">
              Análisis Financiero <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded font-mono">Gemini AI Inside</span>
            </h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 pt-6 space-y-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-[#0F1626] border border-slate-800 rounded-xl p-5 space-y-2">
            ...
            <div className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1">
              <i className="fa-solid fa-chart-pie"></i> {portfolio.length} activos registrados en Supabase
            </div>
          </div>
          ...
          <div className="bg-[#0F1626] border border-slate-800 rounded-xl p-5 space-y-2">
            ...
            <div className="flex items-center gap-2">
              <span className={`w-3 h-3 rounded-full ${
                aiEvaluator.statusColor === 'red' ? 'bg-rose-500' :
                aiEvaluator.statusColor === 'amber' ? 'bg-amber-400' : 'bg-emerald-400'
              } animate-pulse`}></span>
              <span className="text-sm font-bold text-slate-100">{aiEvaluator.riskLevel}</span>
            </div>
            <p className="text-[11px] text-slate-300 leading-relaxed">
              {aiEvaluator.suggestion}
            </p>
          </div>
          ...
          <div className="bg-[#0F1626] border border-slate-800 rounded-xl p-5 space-y-2">
            ...
            <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden flex mt-1">
              <div className="bg-sky-500 h-full w-[65%]" title="CEDEARs / Global"></div>
              <div className="bg-amber-500 h-full w-[35%]" title="Merval / Local"></div>
            </div>
        ...
        {/* TABLA PORTFOLIO + PANEL NOTICIAS AI */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <section className="lg:col-span-7 space-y-6">
            <div className="bg-[#0F1626] border border-slate-800 rounded-xl p-5 sm:p-6 shadow-xl">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400">Monitoreo cuantitativo e integración Supabase</p>
                </div>
                <button
            ...
            {/* RADAR DE OPORTUNIDADES GENERADO EN VIVO POR GEMINI */}
            <div className="bg-[#0F1626] border border-slate-800 rounded-xl p-5 sm:p-6 space-y-4">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <div>
                  <h3 className="text-sm font-bold text-slate-100">Oportunidades Destacadas (Radar Gemini)</h3>
                  <p className="text-[11px] text-slate-400">Análisis y proyección cuantitativa generados por la IA</p>
                </div>
                ...
                {opportunities.map((opp) => (
                  <div key={opp.ticker} className="bg-slate-900/80 border border-slate-800 rounded-lg p-3 space-y-2 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="mt-2">
                        <span className="text-[10px] text-slate-400 block font-medium">Potencial Estimado Gemini:</span>
          ...
          {/* Panel Noticias Analizadas por Gemini */}
            ...
            <div className="bg-[#0F1626] border border-slate-800 rounded-xl p-5 sm:p-6 shadow-xl">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3 mb-4">
                ...
                <span className="text-[10px] text-emerald-400 font-mono flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span> GEMINI EN VIVO
                </span>
              </div>
